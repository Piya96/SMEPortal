...
namespace SME.Portal.Notifications
{
    public class AppNotifier : PortalDomainServiceBase, IAppNotifier
    {
        ...
        public async Task OnboardedOwnerCompanyAsync(User user)
        {
            await _notificationPublisher.PublishAsync(
                AppNotificationNames.WelcomeToTheApplication,
                new MessageNotificationData(L("CongratsForOnboardingCompleteFirstApplication")),
                severity: NotificationSeverity.Success,
                userIds: new[] { user.ToUserIdentifier() }
            );
        }
