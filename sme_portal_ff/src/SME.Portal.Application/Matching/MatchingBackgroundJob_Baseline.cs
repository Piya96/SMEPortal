...
{
	public class MatchingBackgroundJob_Baseline : BackgroundJob<MatchApplicationDto>, ITransientDependency
	{
		...
		public MatchingBackgroundJob_Baseline(
		...
		public override void Execute(MatchApplicationDto request)
		{
			try
			{
				...
				using(UnitOfWorkManager.Current.SetTenantId(request.TenantId))
				{
					...
					var appCriteriaDto = new ApplicationCriteriaDto_Baseline(criteria, companyDto, ownerDto, listItems, request.TenancyName);
				...
				Logger.Info($"AppId:{request.ApplicationId} : Queuing HubSpot Sync Job - FunderSearch and Company properties sync");
			...
			{
				Logger.Error($"AppId:{request.ApplicationId} : MatchingBackgroundJob failed with exception.Message:{x.Message}");
			}
		}
		...
		private MatchResponseDto MatchCriteria(
			List<ExclusionDto> exclusions,
			ApplicationCriteriaDto_Baseline app,
			IEnumerable<FinanceProductCriteriaDto> financeProducts
		)
		{
		...
		private MatchResponseDto HandleFinanceForSub(
			ApplicationCriteriaDto_Baseline app,
			MatchResponseDto finalMatches,
			List<ExclusionDto> exclusions
		)
		{
			// cash for invoice
		...
		public MatchResponseDto MatchOwnership(
			IEnumerable<FinanceProductCriteriaDto> lendersForMatching,
