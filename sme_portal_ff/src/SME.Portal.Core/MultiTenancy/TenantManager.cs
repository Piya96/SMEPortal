...
using SME.Portal.MultiTenancy.Demo;
...
namespace SME.Portal.MultiTenancy
{
    ...
    public class TenantManager : AbpTenantManager<Tenant, User>
    {
            ...
            string emailActivationLink)
        {
            ...
            {
                throw new UserFriendlyException(LocalizationManager.GetString(PortalConsts.LocalizationSourceName, "TrialWithoutEndDateErrorMessage"));
            }
        ...
        public async Task CheckEditionAsync(int? editionId, bool isInTrialPeriod)
        {
            ...
            var error = LocalizationManager.GetSource(PortalConsts.LocalizationSourceName).GetString("FreeEditionsCannotHaveTrialVersions");
        ...
        public List<string> GetAllTenancyNames()
        {
            return base.TenantRepository.GetAll().Where(a => a.IsDeleted == false && a.IsActive).Select(a => a.TenancyName).ToList();
        }
    }
}
