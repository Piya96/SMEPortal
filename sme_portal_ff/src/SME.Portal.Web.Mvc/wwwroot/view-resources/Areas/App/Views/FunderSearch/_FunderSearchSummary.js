...
app.getFunding.summary = {};

function funderSearchLoadSummary() {
    ...
    (function (summary) {
        ...
        let _financialInfo = {};
        // FinancialInfo ( _StepIncomeProfitability.cshtml )
        _financialInfo[annualturnover] = { control: app.json.select(annualturnover), parentId: '' };
        _financialInfo[bank] = { control: app.json.select(bank), parentId: '' };
        _financialInfo[bankaccservices] = { control: app.json.checkbox(bankaccservices), parentId: '' };
        _financialInfo[businessloans] = { control: app.json.radio(businessloans), parentId: '' };
        _financialInfo[whoistheloanfrom] = { control: app.json.select(whoistheloanfrom), parentId: '' };
        _financialInfo[businesstxpersonalacc] = { control: app.json.radio(businesstxpersonalacc), parentId: '' };
        _financialInfo[personalbank] = { control: app.json.select(personalbank), parentId: '' };
        _financialInfo[haselecaccsystems] = { control: app.json.radio(haselecaccsystems), parentId: '' };
        _financialInfo[elecaccsystems] = { control: app.json.select(elecaccsystems), parentId: '' };
        ...
        _financialInfo[payrollsystems] = { control: app.json.select(payrollsystems), parentId: '' };
        ...
        function Init(mc) {
            let matchCriteria = JSON.parse(mc.substring(1, mc.length - 1));
        ...
        function RenderFinancialInfo() {
            ...
            function resetRowDivs() {
                $("#id-div-annualturnover").hide();
                $("#id-div-bank").hide();
                $("#id-div-bankaccservices").hide();
                $("#id-div-businessloans").hide();
                $("#id-div-whoistheloanfrom").hide();
                $("#id-div-businesstxpersonalacc").hide();
                $("#id-div-personalbank").hide();
                $("#id-div-haselecaccsystems").hide();
                $("#id-div-elecaccsystems").hide();
                $("#id-div-elecaccsystemother").hide();
                $("#id-div-haspayroll").hide();
                $("#id-div-payrollsystems").hide();
                $("#id-div-payrollsystemother").hide();
                $("#id-div-hasinvestedownmoney").hide();
                $("#id-div-businessisprofitable").hide();
                $("#id-div-businesshascolateral").hide();
            }

            function resetBankAccServices() {
                for (let i = 0; i < 10; i++) {
                    let idDst = ('#id-bankaccservices-' + (i + 1).toString());
                    $(idDst).hide();
                }
            }
            ...
            enumFinancialInfoControls(function (_control) {
                ...
                function renderCheckboxSummary() {
                    ...
                    function renderCheckboxSummary_bankaccservices() {
                        $('#id-div-bankaccservices').show();
                        let valArr = control.getAll(true);
                        for (let i = 0, max = valArr.length; i < max; i++) {
                            let idSrc = ('#id-val-' + valArr[i]);
                            let idDst = ('#id-bankaccservices-' + (i + 1).toString());
                            $(idDst).text($(idSrc).text());
                            $(idDst).show();
                        }
                    }
                    switch (control.name) {
                        case 'bankaccservices':
                            renderCheckboxSummary_bankaccservices();
                            break;
                    }
                }
                ...
                let div = ('#id-div-' + control.name);
                let id = ('#id-' + control.name);
                $(div).show();
                switch (control.type) {
                    case 'checkbox':
        ...
        function RenderFundingRequirement(matchCriteria, appId, dataArr) {
            appId = '-' + appId;

            function pushTripple(name, value, data) {
                dataArr.push(
            ...
            function enumFundingRequirementControls(cb) {
                ...
                function showWorkingCapitalControls() {
                    ...
                    function showCashAdvanceForAnInvoiceControls() {
                        ...
                        switch (_fundingRequirement.customeragreed.control.control.val()) {
                            case 'No':
                    ...
                    function showCashForRetailersWithCardMachineControls() {
                        ...
                        switch (_fundingRequirement.regularmonthlyincome.control.control.val()) {
                            case 'Yes':
            ...
            enumFundingRequirementControls(function (_control) {
                ...
                function renderInputSummary() {
                    ...
                    function renderAmountSummary() {
                        ...
                        $(id).text(val);
                        //pushTripple(control.name, control.val, control.val);
                    }
                ...
                function renderRadioSummary() {
                    ...
                    function renderGUIDSummary() {
                        let val = control.val();
                        //let srcId = '#id-val-' + val;
                        let dstId = ('#id-' + control.name + appId);
                        //let srcVal = $(srcId).text();
                        let srcVal = app.listItems.get(parentId, val);
                        $(dstId).text(srcVal);

                        //pushTripple(control.name, val, $(srcId).text());
                    }
                ...
                function renderCheckboxSummary() {
                    ...
                    function renderGUIDSummary() {
                        ...
                        for (let i = 0, max = valArr.length; i < max; i++) {
                            ...
                            //let srcVal = $(idSrc).text();
                            let srcVal = app.listItems.get(parentId, valArr[i]);
                            $(dstId).text(srcVal);
                            $(dstId).show();

                            //pushTripple(control.name, valArr[i], $(idSrc).text());
                        }
                    }
                ...
                function renderSelectSummary() {
                    ...
                    function renderIndustrySectorSummary() {
                        ...
                        //pushTripple(control.name, industrySectorKey, innerText);
                    }
        ...
        function Test(funderSearches) {
            if (funderSearches != null) {
                let dataArr = [];
                let fs = funderSearches;
                fs = JSON.parse(fs);
                for (const name in fs) {
                    RenderFundingRequirement(fs[name], name, dataArr);
                }
            }
        }
        ...
        summary.renderFinancialInfo = RenderFinancialInfo;

        summary.renderFundingRequirement = RenderFundingRequirement;
