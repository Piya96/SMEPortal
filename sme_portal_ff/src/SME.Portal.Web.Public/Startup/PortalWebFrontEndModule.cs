...
namespace SME.Portal.Web.Public.Startup
{
    [DependsOn(
        typeof(PortalWebCoreModule)
    )]
    public class PortalWebFrontEndModule : AbpModule
    {
        private readonly IConfigurationRoot _appConfiguration;

        public PortalWebFrontEndModule(IWebHostEnvironment env, PortalEntityFrameworkCoreModule abpZeroTemplateEntityFrameworkCoreModule)
        {
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(PortalWebFrontEndModule).GetAssembly());
        }
    }
}
