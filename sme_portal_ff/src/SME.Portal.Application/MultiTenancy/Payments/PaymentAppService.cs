...
{
    [Authorize]
    public class PaymentAppService : PortalAppServiceBase, IPaymentAppService
    {
        ...
        [Authorize(AppPermissions.Pages_Administration_Tenant_SubscriptionManagement)]
        ...
        public async Task<PagedResultDto<SubscriptionPaymentListDto>> GetSmePaymentHistory(GetPaymentHistoryInput input)
        {
            var user = await UserManager.GetUserAsync(AbpSession.ToUserIdentifier());

            var query = _subscriptionPaymentRepository.GetAll()
                .Include(sp => sp.Edition)
                .Where(sp => sp.TenantId == AbpSession.GetTenantId() && sp.UserId == user.Id && (sp.Status == SubscriptionPaymentStatus.Paid || sp.Status == SubscriptionPaymentStatus.Completed))
                .OrderBy(input.Sorting);
        ...
        public async Task SetAsPaid(long paymentId, string externalPaymentId, string externalPaymentToken, string invoiceNo)
        {
            ...
            if (!string.IsNullOrEmpty(externalPaymentToken))
                payment.ExternalPaymentToken = externalPaymentToken;

            await _subscriptionPaymentRepository.UpdateAsync(payment);
        }

        public async Task SetAsCancelled(long paymentId)
        {
            var payment = await _subscriptionPaymentRepository.GetAsync(paymentId);
        ...
        [AllowAnonymous]
        public string GetPaymentDescription(EditionPaymentType editionPaymentType, PaymentPeriodType? paymentPeriodType, string targetEditionName, bool isRecurring)
        {
