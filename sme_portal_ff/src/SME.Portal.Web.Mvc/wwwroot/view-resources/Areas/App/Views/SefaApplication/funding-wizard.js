...
function fundingWizardLoadWizard() {
    ...
    (function (getFunding) {
        ...
        function retrieveSummaryToPDF(cb) {
            ...
            let status = AddStatus();
            $.ajax({
                url: '/App/SefaApplication/GetFunderSearchSummaryPdf',
                type: "POST",
                data: JSON.stringify(data),
                contentType: "application/json",
                success: function (data) {
        ...
        function submitCommon(partial, applicationId, propJson, controllerFn, cb) {

            let formArray = $('form').serializeArray();
            let matchCriteria = formArray;//propJson.matchCriteriaJson.concat(formArray);
            ...
            $.ajax({
                ...
                url: "/App/SefaApplication/" + controllerFn,
                data: JSON.stringify(data),
                success: function (data) {
                    if (cb != null) {
                        cb(status, data);
        ...
        let submit = function (applicationId, propJson, cb) {
            submitCommon(false, applicationId, propJson, 'Submit', function (status, data) {
                cb(status, data);
            });
        }


        let submitPartial = function (applicationId, propJson, cb) {
            submitCommon(true, applicationId, propJson, 'Submit', function (status, data) {
                cb(status, data);
            });
        }
    ...
    (function (getFunding) {
        ...
        function savePartialForm(cb) {
            ...
            app.getFunding.crud.submitPartial(_application.id, null, function (status, data) {
                KTApp.unblockPage();
                cb(AddStatus());
            });
        }
        ...
        function neglect(page, next, cb) {
            ...
            function onSave(page, cb) {
                switch (page) {
                    case Pages.FinancialInfo:
                        savePartialForm(function (status) {
                        });
                        break;
    ...
    KTWizardFunding = function () {
        ...
        var initValidation = function () {
            ...
            let documentationMap = new Map();
            map(documentationMap, '5aaf6d123a022727ec3b587a', null, 'id-company-registration-reg-doc-message');
            map(documentationMap, '60549c63a2b3b4fb7c24bb34', null, 'id-cipc-annual-return-doc-message');
            map(documentationMap, '5ab0cdb53efed8141436959b', null, 'id-cert-owners-reg-doc-message');
            map(documentationMap, '5b2a54ecb958c008605883ff', null, 'id-proof-of-addr-reg-doc-message');
            map(documentationMap, '60549db9e3032aa608a98cc1', null, 'id-bee-certificate-doc-message');
            map(documentationMap, '5aaf6d193a022727ec3b587b', null, 'id-bank-statements-reg-doc-message');
            map(documentationMap, '5ab0ce1d3efed814143695a1', null, 'id-months-12-budget-reg-doc-message');
            map(documentationMap, '5ab0cdd73efed8141436959d', null, 'id-assets-liabilities-reg-doc-message');
            map(documentationMap, '5ab0cdc33efed8141436959c', null, 'id-tax-clearance-reg-doc-message');
            map(documentationMap, '5ab0cde13efed8141436959e', null, 'id-business-plan-reg-doc-message');
            map(documentationMap, '5ab0cdfc3efed814143695a0', null, 'id-management-account-reg-doc-message');
            map(documentationMap, '5ab0cded3efed8141436959f', null, 'id-annual-fin-atatements-reg-doc-message');
            ...
            function addDocumentationValidation() {
                let validation = FormValidation.formValidation(
                    document.getElementById('company-registration-documents'),
                    //formEl,
                    {
                        fields: {
                            '5aaf6d123a022727ec3b587a': {
                                validators: {
                                    notEmpty: {
                                        message: 'Select on of 3'
                                    }
                                }
                            },
                            '60549c63a2b3b4fb7c24bb34': {
                                validators: {
                                    notEmpty: {
                                        message: 'Select on of 3'
                                    }
                        ...
                        plugins: {
                            ...
                            messages: new FormValidation.plugins.Message({
                                ...
                                container: function (field, element) {
                                    ...
                                    //default:
                                    //{
                                    let data = documentationMap.get(field);
                                    if (data != null) {
                                        return document.getElementById(data.div);
                                    } else {
                                        return document.getElementById('id-step-5-messages');
                                    }
                                    //}
                                    //}
                                }
    ...
    function pageChangeCallback(prev, next) {
        ...
        function pageChangeSummary(foreward) {
            ...
            KTWizardFunding.setFunderSearchJson(arr[0].concat(arr[1].concat(arr[2].concat(arr[3]))));
        }
        ...
        switch (next) {
            ...
            case app.getFunding.wizard.Pages.Documentation:
                pageChangeFunderEssentials(next > prev);
                break;
    ...
    let _fananceForBackgroundHighlight = "#e89843";// Darker alice blue.
    ...
    $("input[type='radio']").click(function () {
        ...
        } else if (this.name == 'doyouknowyourpersonalcreditscore') {
            let perRadioValue = $("input[name='doyouknowyourpersonalcreditscore']:checked").val();

            if (perRadioValue) {
                if (perRadioValue === "Yes") {
                    $("#personal-credit-report-link").show("slow");
                } else {
                    $("#personal-credit-report-link").hide(hideSpeed);
                }
            }
        } else if (this.name == 'wanttouploadbusbankstatements') {
            let perRadioValue = $("input[name='wanttouploadbusbankstatements']:checked").val();

            if (perRadioValue) {
                if (perRadioValue === "Yes") {
                    $("#upload-business-bank-statements").show("slow");
                } else {
                    $("#upload-business-bank-statements").hide(hideSpeed);
                }
            }
        }
    });
    ...
    // owners-contribution - currency formatting
    $('#owners-contribution').on('input', function (event) {
