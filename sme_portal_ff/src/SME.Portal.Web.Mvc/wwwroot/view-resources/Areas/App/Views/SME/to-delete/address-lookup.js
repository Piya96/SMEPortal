...
function fillComponents(id) {
    let place = autocomplete.getPlace();
    $('#' + id).val(place.formatted_address);
}

function fillInAddress_user() {
    fillComponents('id-input-user-profile-registered-address');
}
...
function fillInAddress_owner() {
    fillComponents('id-input-owner-profile-registered-address');
}

function initAutocomplete_owner() {
    autocomplete = new google.maps.places.Autocomplete(document.getElementById('id-input-owner-profile-registered-address'), { types: ['geocode'] });

    autocomplete.setFields(['formatted_address']);

    autocomplete.addListener('place_changed', fillInAddress_owner);
